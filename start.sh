...
# ── INITIALISATION ───────────────────────────────────────────
echo ""
echo "━━━ INITIALISATION — Clonage des modules source ━━━━━━━━━"
 
info "Vérification des modules source..."
MODULES="amc_annuaire amc_clients amc_comptes amc_composite amc_proxy amc_configserver"
ALL_PRESENT=true
 
for module in $MODULES; do
  if [ ! -f "src/AMSC/${module}/pom.xml" ]; then
    ALL_PRESENT=false
    break
  fi
done
...
if [ "$ALL_PRESENT" = false ]; then
  info "Clonage des modules manquants..."
  cd src/AMSC
  for module in $MODULES; do
    if [ ! -d "${module}" ]; then
      info "Clonage de ${module}..."
      git clone --depth=1 https://github.com/hialmar/${module} ${module}
    else
      info "${module} déjà présent "
    fi
  done
  cd ../..
else
  info "Tous les modules source sont déjà présents "
fi
 
# Vérification pom.xml
POM_COUNT=$(find src/AMSC -maxdepth 2 -name "pom.xml" | wc -l)
if [ "$POM_COUNT" -lt 7 ]; then
  error "Il manque des pom.xml ($POM_COUNT/7) — vérifiez src/AMSC/"
fi
info "Modules source OK ($POM_COUNT pom.xml trouvés) "
...
command -v java     >/dev/null 2>&1 || error "java non installé → https://adoptium.net/"
command -v mvn      >/dev/null 2>&1 || error "maven non installé → https://maven.apache.org/"
...
# ArgoCD CLI — installation automatique si absent
if ! command -v argocd >/dev/null 2>&1; then
  warning "ArgoCD CLI non installé — installation automatique..."
  curl -sSL -o /tmp/argocd \
    https://github.com/argoproj/argo-cd/releases/latest/download/argocd-linux-amd64
  chmod +x /tmp/argocd
  sudo mv /tmp/argocd /usr/local/bin/argocd 2>/dev/null || \
    mv /tmp/argocd ~/.local/bin/argocd 2>/dev/null || \
    mv /tmp/argocd ./argocd
  info "ArgoCD CLI installé "
fi
...
helm repo update
# Nettoyer les opérations bloquées si nécessaire
kubectl delete secret -n argocd \
  -l owner=helm,status=pending-upgrade 2>/dev/null || true
...
echo ""
echo "--- ArgoCD ---"
argocd app get miage-bank 2>/dev/null || \
  kubectl get application miage-bank -n argocd 2>/dev/null || true
echo ""
echo "============================================================"
